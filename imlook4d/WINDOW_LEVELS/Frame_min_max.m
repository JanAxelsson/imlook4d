function Frame_min_max(hObject, eventdata, handles)
...
%     slice=round(get(imlook4d_handles.SliceNumSlider,'Value'))
    ...
    minValue=min(min(min( imlook4d_handles.image.Cdata(:,:,:,frame)) ))
    maxValue=max(max(max( imlook4d_handles.image.Cdata(:,:,:,frame)) ))
