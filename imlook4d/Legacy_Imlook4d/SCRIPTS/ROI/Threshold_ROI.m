...
%
% Threshold is specified in percent  (% sign at end of value) or absolute value
% IF percent threshold : threshold is calculated as (THRESH - BACKGROUND)*PERCENT + BACKGROUND
% If absolute value :    threshold = absolute value
%
% Inputs :
% - highest value to include (per cent of max, or absolute value)
% - threshold (per cent of max, or absolute value)
% - first slice
% - last slice 
% - background value (default zero, auto from first ROI if value is 'auto', or Reference ROI if exists)
%
%
% Jan Axelsson
...
% INITIALIZE
    ...
    defaultanswer = RetriveEarlierValues('Threshold', {'100%','40%', '1' , 'end', '0'} ); 
    ...
    try
        ...
        defaultanswer{5} = num2str(background);
    catch
    end
...
%             currentMinThresholdLevel=minThresholdLevel;
        ...
        % If percent threshold,  calculate threshold value (as percent of max in this frame)
            ...
            if strcmp( maxThresholdLevel(end), '%')
                ...
                maxInputPercent = str2num(maxThresholdLevel(1:end-1)) ;
                currentMaxThresholdLevel = 0.01 * maxInputPercent * ( maxLevel - background ) + background;
            else
            ...
            if strcmp( minThresholdLevel(end), '%')
                ...
                minInputPercent = str2num(minThresholdLevel(1:end-1)) ;
                currentMinThresholdLevel = 0.01 * minInputPercent * ( maxLevel - background ) + background;
            else
