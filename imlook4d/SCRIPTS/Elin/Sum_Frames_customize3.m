...
%     historyDescriptor=str2mat(answer(2)); % Make a descriptor to prefix new window title with
    ...
    for i=1:nr_new_frames
        for j=1:xls_matrix(i, 2)
            ...
            decay_factor=2^(-imlook4d_time(a+j)/imlook4d_current_handles.image.halflife);
            new_image(:, :, :, i)=new_image(:, :, :, i)+imlook4d_Cdata(:, :, :, a+j)*decay_factor/xls_matrix(i, 2);
...
%     new_image(:, :, :, nr_new_frames)=new_image(:, :, :, nr_new_frames)/last_frame_sum;
    ...
    for i=1:nr_new_frames
        decay_factor2=2^(-new_imlook4d_time(i)/imlook4d_current_handles.image.halflife);  
        new_image(:, :, :, i)=new_image(:, :, :, i)/decay_factor2;
    end
