...
% INITIALIZE
    ...
    % Reformat  
        ...
        imlook4d_Cdata=permute(imlook4d_Cdata, [2 3 1 4]);  % Image
        imlook4d_ROI=flipdim(imlook4d_ROI,3); % Flip upside down in z-direction
        imlook4d_ROI=permute(imlook4d_ROI, [2 3 1 4]);      % ROI
    ...
    % Store view type
        imlook4d_current_handles.image.plane='Sagital';
        
    % Set title
        WindowTitle(['(' imlook4d_current_handles.image.plane ')'],'append') 

    % Record history (what this image has been through)
        imlook4d_current_handles.image.history=['SAG' '-' imlook4d_current_handles.image.history  ];
