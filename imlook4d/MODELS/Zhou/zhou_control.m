function varargout  =  zhou_control(varargin)

% zhou_CONTROL M-file for zhou_control.fig
%      zhou_CONTROL, by itself, creates a new zhou_CONTROL or raises the existing
%      singleton*.
%
%      H  =  zhou_CONTROL returns the handle to a new zhou_CONTROL or the handle to
%      the existing singleton*.
%
%      zhou_CONTROL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in zhou_CONTROL.M with the given input arguments.
%
%      zhou_CONTROL('Property','Value',...) creates a new zhou_CONTROL
%      or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before zhou_control_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to zhou_control_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help zhou_control

% Last Modified by GUIDE v2.5 17-Aug-2016 13:52:22

% Begin initialization code - DO NOT EDIT
gui_Singleton  =  1;
gui_State  =  struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @zhou_control_OpeningFcn, ...
                   'gui_OutputFcn',  @zhou_control_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before zhou_control is made visible.
function zhou_control_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to zhou_control (see VARARGIN)

% Choose default command line output for zhou_control
...
%
   disp('Entered zhou_control_OpeningFcn');
    ...
    %            
            ...
            try
                ...
                % Set list-box
                set(handles.referenceDataEdit,'String', imlook4d_handles.model.Zhou.referenceData);

                % Set GUI according to imlook4d model handles
                imlook4d_handles.model.Zhou.referenceData   % Save TACT curve 
                set(handles.startFrameSlider,'Value', imlook4d_handles.model.Zhou.startFrame);
                set(handles.startFrameEdit  ,'String',imlook4d_handles.model.Zhou.startFrame);
                set(handles.endFrameSlider  ,'Value', imlook4d_handles.model.Zhou.endFrame);
                set(handles.endFrameEdit    ,'String',imlook4d_handles.model.Zhou.endFrame);
                
                
                set(handles.referenceDataEdit    ,'String',num2cell(imlook4d_handles.model.Zhou.referenceData));
           catch
                disp('First time opening zhou from this instance of imlook4d');
...
% UIWAIT makes zhou_control wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout  =  zhou_control_OutputFcn(hObject, eventdata, handles) 
...
% handles    structure with handles and user data (see GUIDATA)
        ...
        % Define imlook4d callback function
            imlook4d_handles.model.functionHandle = @zhou;              % CHANGE THIS ONE
            
        % Save into imlook4d handles
            %imlook4d_handles.model.Zhou.referenceData = activity(activeROI,:);   % Save TACT curve 
            imlook4d_handles.model.Zhou.startFrame = round(get(handles.startFrameSlider,'Value'));
            endFrame = round(get(handles.endFrameSlider,'Value'));
            imlook4d_handles.model.Zhou.endFrame = endFrame;
        ...
        % Determine if slope/intercept image
            ...
            if (get(handles.interceptRadiobutton,'Value') == 1)
                imlook4d_handles.model.Zhou.type = 'intercept';
            end  
        ...
        % Get reference data from text field, line-by-line
            ...
            imlook4d_handles.model.Zhou.referenceData = referenceData;
