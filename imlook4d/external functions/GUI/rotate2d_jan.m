function out=rotate2d_jan(varargin)
...
%         'Style','text','BackgroundColor', fig_color,'HandleVisibility','off');
    ...
    % JAN -- make textbox for angle
    rdata.ModeStateData.textBoxText = uicontrol('Parent',hFig,'Units','Pixels',...
        'Position',[2 2 130 20],'Visible','on', ...
        'Style','edit','BackgroundColor', fig_color,'HandleVisibility','off', ...
        'String','0', ...
        'Callback', @angleEditCallback);
    
end

% JAN ---------------------------------------------------------------%
function angleEditCallback(hObject,event)
    hManager = uigetmodemanager(gcf);
    angleText = get(hManager.CurrentMode.ModeStateData.textBoxText,'String');
...
if strcmpi(sel_type,'open')
    ...
    % JAN -- set to zero
    hManager = uigetmodemanager(gcf);
    set(hManager.CurrentMode.ModeStateData.textBoxText,'String','0');
...
if(rotaObj.ModeStateData.textState)
    ...
    if ~isvalid(rotaObj.ModeStateData.textBoxText)
...
%JAN set(rotaObj.ModeStateData.textBoxText,'Visible','off');
...
% JAN fixate elevation to 90 degrees
new_azel(2) = 90;
